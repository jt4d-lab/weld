...
Пресета два, и выбор между ними — про то, насколько правила склонны ругаться на живом коде. Каждый
включает свои правила как `'error'` без опций и приносит вместе с ними требования к `settings.weld`
...
То же плюс правила, которые технически допустимую запись всё равно считают нарушением, — на
существующем коде такие требуют осознанной уборки, а не одной правки импорта. Набор для проектов,
которые заводят подход с нуля либо уже прошли уборку и хотят держать границы жёстко.

Пока реализованных правил мало, составы пресетов совпадают; расходиться они начнут по мере появления
правил.

Если не подходит ни один — набор правил нужен свой, — подключается сам плагин, а правила включаются
поштучно:
