...
| имя слоя не имя директории (`/`, `.`, `..`)           | конкретный элемент           |
| обычное имя `unknown` или `module` (без `@`)          | конкретный элемент           |
...
Два **обычных** имени зарезервированы потому, что ими же `plainLayerName` называет спец-слои:
`unknown` дал бы ровно тот же квалификатор, что и `@unknown` (проверка «слой не объявлен» перестала
бы срабатывать), а `module` — отдельный ключ `root:module`, неотличимый в сообщениях от модуля
целиком. `.` и `..` отвергаются вместе с `/`: сегментом пути они не бывают, и такой слой молча не
существовал бы.

Совет вместо зарезервированного имени зависит от списка (правка четвёртой итерации ревью): в
`layers` вместо `module` предлагается `@modules`, а в `moduleLayers` — взять другое имя, потому что
`@modules` там отвергает соседняя проверка, а место модулям объявляется один раз в `layers` и
действует на любой глубине. У `unknown` совет один на оба списка: `@unknown` законен в каждом из
них.
...
6. `layerOf(target, schema, 'target')`; оба владельца не `null` и равны → пропуск (внутренний
   импорт);
7. позиции цели нет в схеме (порядок веток — тот же, что в `undeclaredTarget`):
    - цель не `module:unknown` → `undeclaredTargetLayer` с советом по самому слою: `root:unknown` →
      `'@unknown' in layers`, `module` (схема без `@modules`) → `'@modules' in layers`;
    - цель `module:unknown`, но `@modules` в схеме **нет** → `undeclaredTargetLayer` с `{{layer}}` =
      `module` и `{{declare}}` = `'@modules' in layers`. Проверяется **раньше** своего модуля: для
      схемы без `@modules` модулей не существует вовсе, и совет про `moduleLayers` вёл бы в конфиг,
      который разбор схемы отвергает исключением. Ветка «свой модуль» при такой схеме недостижима —
      источник внутри модуля сам оказывается слоем вне схемы, и его импорты не проверяются;
...
8. `first(toLayer) < last(fromLayer)` → нарушения нет; иначе репорт: `horizontalDependency` при
   совпадении квалифицированных имён (совет — смежный повтор в названном списке: `moduleLayers` для
   слоя модуля, иначе `layers`); при совпадении **простых** имён у разных квалифицированных слоёв
   (`@unknown` двух уровней) — `illegalDependencyAcrossLevels` с уровнем каждого конца; иначе
   `illegalDependency`.
...
Последние две строки — единственная пара, которую простое имя не различает, поэтому нарушение
направления **между** ними идёт отдельным сообщением (`illegalDependencyAcrossLevels`): к каждому
концу добавляется уровень (`outside modules` / `inside a module`). Совпадение простых имён на одном
уровне означало бы один и тот же слой (горизонталь), а обычное имя сразу в обоих списках отвергает
разбор схемы, — других таких пар не бывает.
...
`horizontalDependency` сформулировано нейтрально (`must not import from another '<слой>'`, а не «из
другой директории»): условие — совпадение имён при разных владельцах, а это возможно не только для
слоёв модулей (`root:app` → `root:app` из-за deepest, два неразмеченных файла при объявленном
`@unknown` — у них владельца нет вовсе, и горизонталью оказывается даже сосед по директории). Совет
называет список, в котором объявляется смежный повтор: слой модуля повторяется в `moduleLayers`, и
тот же повтор в `layers` разбор схемы отверг бы.
...
➕ по итогам ревью (шестая итерация) совет вместо зарезервированного `module` ветвится по
`hasModules` и во **втором** списке. Имена проверяются раньше схемы целиком, поэтому при
`moduleLayers: ['module']` и `layers` без `@modules` прежний текст утверждал про конфиг то, чего в
нём нет («место модулям объявлено как `@modules` в `layers`»), а «возьми другое имя» упиралось в
следующую же ошибку — `moduleLayers` некуда класть. Теперь в этой ветке названы оба шага. Там же оба
места в конфиге поехали одним словарём (`SchemaContext.source` по ключу `LayerList`) вместо пары
строк, которую валидаторы передавали друг другу позиционно: имя места выбирается тем же ключом, что
и совет. Правки: `src/settings/layers.ts`, `layers.test.ts`, `docs/settings.md`.
...
➕ по итогам ревью (шестая итерация) правило «override выигрывает, иначе секция, источник называется
так-то» осталось в одном месте. `pickSetting(name, settings, overrides)`, заведённый здесь для
схемы, стал единственной реализацией: через него пошли `getRepoRoot`, `getAliasesBaseUrl` и
`readAliases` (было по своей ветке `!== undefined` и по паре литералов `options.<имя>` /
`settings.weld.<имя>` на каждую настройку), а `hasAliases`/`hasLayers` — через общий `hasSetting`
поверх него. Тексты ошибок и ключ кэша алиасов не изменились. Правки: `src/settings/weld.ts`,
`src/settings/index.ts` (устаревший заголовок барреля).
...
модуля, и в схеме без `@modules` он недостижим вовсе. Баррель модуля к этому сообщению тоже не
приводит (уточнено по итогам четвёртой итерации ревью): у него владелец — та же директория модуля,
что и у неразмеченного кода, и равные владельцы отсекают такой импорт первым же шагом.

➕ по итогам ревью (третья итерация) набор сообщений вердикта вырос до пяти: простое имя не
различает `@unknown` двух уровней, и при объявленном `@unknown` в обоих списках `illegalDependency`
называл одним именем оба конца импорта («`'@unknown' must not import from '@unknown'`»). Такая пара
получила своё сообщение — `illegalDependencyAcrossLevels`, с уровнем каждого конца; уровень отдаёт
`isModuleLevel` из `src/settings/` (формат квалификатора остаётся знанием одного слоя). Заодно совет
в `horizontalDependency` стал называть список для повтора: для слоя модуля повтор в `layers` разбор
схемы отверг бы. Обе правки отражены в таблице сообщений и шагах 7–8 Technical Details.
...
- [x] написать тесты на границы: формы импорта (`export ... from` — см. правку ниже, `import type` и
      `require()`), файл вне корня репозитория молчит, отсутствие `layers` — исключение на каждом
...
- [x] написать `options.test.ts` по образцу соседа: `options.layers`/`moduleLayers`/`moduleDir`
      ...
      принимаются
- [x] реализовать правило: `createRule(fsHost?)` (шов для тестов), `meta` с шестью сообщениями и
...
➕ по итогам ревью (четвёртая итерация) правило **проверяет реэкспорты**. План и первая реализация
наследовали пропуск `export ... from` у `no-barrel-bypass`, то есть не принимали решение, а получали
его от общего визитора. Для направления зависимостей реэкспорт — обычное ребро графа, и его пропуск
делал запрет обходимым переписыванием пары `import` + `export` в одну строку. У соседа ответ
остаётся обратным (баррель собран из реэкспортов — правило репортило бы сам баррель), поэтому обход
включается параметром `createSpecifierVisitor(onSpecifier, { reExports: true })`: по умолчанию он
выключен, и поведение `no-barrel-bypass` не меняется (закреплено тестами с обеих сторон). Баррелям
включение ничего не стоит: у барреля слоя владелец общий с содержимым, у барреля модуля работает
диапазон `module`. Правки: `src/imports/nodes.ts` и его тесты, `rule.test.ts` обоих правил, раздел
«Что проверяется» страницы правила (пункт из «Что правило сознательно не проверяет» снят), страница
`no-barrel-bypass` (решение названо её собственным) и `CLAUDE.md`.

➕ по итогам ревью (пятая итерация) общий визитор обходит и `import x = require('...')`. Специфаер
этой формы лежит в `TSExternalModuleReference`, а не в `source` и не в аргументе `CallExpression`,
поэтому её не видел ни один слушатель: запись молча обходила **оба** правила — и направление
зависимостей, и баррель. Пробел был и в `master`, то есть правилом не внесён, но страница
`no-barrel-bypass` перечисляла `require('...')` как проверяемую форму, не оговаривая эту запись.
Решение принято в пользу поддержки: место ей ровно в `src/imports/nodes.ts` («где в AST лежит
строка-специфаер»), одна правка закрывает оба правила, а `replaceSpecifier` работает по диапазону
литерала и правит новую форму без изменений (автофикс соседа закреплён тестом). Форма с
модификатором (`export import x = require(...)`) обходится так же и при включённых реэкспортах отчёт
не удваивается: у внешнего `ExportNamedDeclaration` `source` пуст. Правки: `src/imports/nodes.ts` и
его тесты, `rule.test.ts` обоих правил, раздел «Что проверяется» на обеих страницах правил,
`CLAUDE.md`.

➕ там же: совет вместо зарезервированного имени `module` в `layers` стал зависеть от схемы. Прежний
текст («use '@modules'») при уже объявленном `@modules` вёл в конфиг с двумя `@modules`, который
разбор отвергает следующей же проверкой, — то есть единственная пара «сообщение → совет», не
замыкавшаяся на принимаемый конфиг. Теперь совет ветвится по наличию `@modules` в `layers`
(`AdviceContext` в `src/settings/layers.ts`), обе ветви закреплены тестами вместе с проверкой, что
советованный конфиг разбирается. Правки: `src/settings/layers.ts`, `layers.test.ts`,
`docs/settings.md`.

➕ по итогам ревью (шестая итерация) мелочи вокруг правила: в debug-логах слой источника называется
одинаково в обоих местах — `pipeline.ts` логировал сырой `from.layer`, а правило уже схлопнутые
права, и неразмеченный файл модуля появлялся в одном прогоне то как `module:unknown`, то как
`module`; `@/rules/context.js` вернулся в общую группу импортов (как у соседа); проверка «совет
`moduleInternals` разбирается» переехала из `rule.test.ts` (где сидела между наборами `RuleTester`
без участия правила) к остальным таким проверкам в `verdict.test.ts`.
...
- [x] переписать разделы «Схема слоёв» и «Слой файла»: развёртка в плоский список квалифицированных
      имён, `module` как публичный интерфейс модуля и `module:unknown`, две таблицы «путь → слой»
      (файл и цель), владелец вместо «модуля слоя»
- [x] переписать раздел «Горизонтальные связи» (горизонталь — одноимённые слои разных владельцев) и
      пересчитать матрицу «Что разрешено» под развёртку с обрамляющими `module`
- [x] пересобрать блоки Valid/Invalid: `entities/bar.ts` → `entities/foo/...` теперь разрешён,
      добавить примеры на импорт барреля слоя и модуля, на `moduleInternals` и на цель без слоя
- [x] заменить таблицу сообщений: `unknownLayer`/`unknownTargetLayer` → шесть актуальных id,
      проверить все ссылки на прежние id в разделе «Поведение в крайних случаях»
- [x] обновить раздел «Опции правила» (три собственных плюс три общих) и «Подключение» (схема
      обязательна, `files` ограничивает область), снять плашку «правило ещё не реализовано»
- [x] поправить `docs/settings.md`: `@unknown` в `moduleLayers` допустим, `@modules` там — ошибка,
...
- [x] проверить якоря: ссылки на `#поведение-в-краиних-случаях` и на разделы `settings.md` из обеих
      страниц (VitePress транслитерирует `й`→`и`) — проверены по `id`/`href` в собранном сайте
- [x] добавить правило в список правил `README.md` и поправить там абзац «Статус»
- [x] `yarn docs:build` — сайт собирается, ссылки и якоря целы
- [x] run tests - must pass before task 10

➕ в документацию добавлены два поведения, которых в плане не было, но которые есть в коде и тестах:
пустая схема (`layers: []`) — не ошибка конфига, а схема без слоёв (каждый файл получает
`undeclaredLayer`), и файл вне корня репозитория молчит **раньше** проверки схемы, поэтому на нём не
падает и незаданная схема.

### Task 10: Verify acceptance criteria

- [x] сверить реализацию со страницей правила по пунктам: определение слоя для файла и цели,
      внутренний импорт, горизонталь, `@unknown` на двух уровнях, тексты сообщений, отсутствие
      автофикса
- [x] проверить краевые случаи: файл вне корня репозитория, кривая схема, отсутствующая схема,
      `@modules` без `moduleLayers`, модуль без барреля, импорт `..` и `.`
- [x] прогнать полный набор: `yarn verify`
- [x] прогнать `yarn smoke` — регрессия сборки и подключения пакета (само правило скрипт не
      проверяет: оно вне пресетов)
- [x] убедиться, что обе чистые функции покрыты таблицами, а правило — интеграционно

➕ сверка прошла без расхождений: обе таблицы «путь → слой», матрица «Что разрешено» (пересчитана по
`first(B) < last(A)` для схемы страницы с разнесённым `legacy`), все блоки Valid/Invalid, тексты
сообщений и текст `MISSING_SCHEMA` совпали с кодом дословно. Сверялись они со страницей правила —
таблицу сообщений в самом плане ревью потом поймало на расхождении (см. ниже). Единственная дыра в
покрытии — строки `..` и `.` из таблицы целей: `layerOf` проверялся на уже разрешённых путях, а сами
специфаеры через `parseSpecifier` не проходили ни в одном тесте. Закрыто тремя случаями в
`pipeline.test.ts` (внутренний импорт своей директории, `..` из корня слоя в баррель модуля, `..`
вверх по слоям модуля с подстановкой `{{target}}` = `..`).

➕ «модуль без барреля» отдельного теста не потребовал: `rule.test.ts` и `options.test.ts` целиком
работают на `createFakeFsHost([])` — ни одного файла на фейковой ФС нет, поэтому валидный набор с
`@/modules/other` и есть импорт барреля несуществующего модуля.

➕ сверка велась по странице правила, а не по плану, поэтому расхождение самого плана с кодом она не
поймала — его нашло ревью. К моменту третьей итерации в Technical Details разошлись три места, и все
приведены к коду: таблица `meta.messages` (у `horizontalDependency` оставался текст «from a
different `<слой>` directory», а сообщений стало шесть), шаг 7 «Порядка решений» (не было ветки
«цель внутри модуля при схеме без `@modules`», которая проверяется раньше своего модуля) и таблица
валидации схемы (не было зарезервированных обычных имён `unknown`/`module` и отсева `.`/`..`).
Правило «реализация отклонилась — правим план» с тех пор относится и к правкам, приходящим из ревью,
а не только к правкам внутри задач.

### Task 11: [Final] Update documentation

- [x] обновить `CLAUDE.md`: новое правило в «Текущем состоянии», `src/settings/layers.ts` в описании
...
- [x] проверить, нужна ли правка `docs/core-concepts/` — у правила должен быть парный раздел в
      документации подхода (идея повторов-диапазонов описана только в `docs/drafts/layers.md`)
- [x] переместить план в `docs/plans/completed/` — выполняет харнесс после всех фаз; вручную план не
      двигается, иначе его перестают находить последующие фазы. `yarn docs:build` с планом в
      `docs/plans/` проходит

➕ парный раздел у правила есть — `docs/core-concepts/direction.md` («Однонаправленность
зависимостей»), на него страница правила и ссылается. Не хватало только механики повторов: правило
считает диапазоны, а документация подхода про них молчала. Поэтому в `docs/core-concepts/layers.md`
(«Слои задаёт проект» — там же, где решение «допустимые зависимости определяет проект») добавлено
третье правило «Отступление от направления записывается повтором слоя» и пример с разнесённым
`legacy`; развёрнутый черновик в `docs/drafts/layers.md` остаётся черновиком.
`docs/core-concepts/fractal-modules.md` на момент этой записи был заголовком-заглушкой; статья
написана в этой же ветке позже, по итогам первой итерации ревью (коммит `c5bdba3`) — на неё
ссылается страница правила.

➕ в `CLAUDE.md` заодно исправлено то, что разошлось с веткой помимо правила: «Текущее состояние»
считало документацию подхода несуществующей (статьи `docs/core-concepts/` уже в `docs/`, черновики —
в `docs/drafts/`) и обещало, что формат настройки слоёв ещё не зафиксирован. Добавлен раздел про
само правило (`core/layer-of.ts`, `core/verdict.ts`, `pipeline.ts`) — решения про deepest, владельца
и цель как она записана в специфаере из кода не читаются.
