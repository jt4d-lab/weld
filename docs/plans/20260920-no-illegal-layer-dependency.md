...
- [x] заменить таблицу сообщений: `unknownLayer`/`unknownTargetLayer` → пять актуальных id,
...
пяти сообщений и текст `MISSING_SCHEMA` совпали с кодом дословно. Единственная дыра в покрытии —
строки `..` и `.` из таблицы целей: `layerOf` проверялся на уже разрешённых путях, а сами специфаеры
через `parseSpecifier` не проходили ни в одном тесте. Закрыто тремя случаями в `pipeline.test.ts`
(внутренний импорт своей директории, `..` из корня слоя в баррель модуля, `..` вверх по слоям модуля
с подстановкой `{{target}}` = `..`).
...
`docs/core-concepts/fractal-modules.md` по-прежнему заголовок-заглушка — отдельная работа, к этому
плану не относящаяся: фрактальность модулей правило использует, но идея старше правила.
