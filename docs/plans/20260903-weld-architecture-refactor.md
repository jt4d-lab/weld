...
- [ ] добавить `get-tsconfig` в `devDependencies` (`package.json`), прогнать установку зависимостей
      ...
      потребителей пакета просто не установится
- [ ] в `env/tsconfig.ts` объявить сигнатуру будущей функции чтения алиасов из `tsconfig.json`
...
- [ ] файл **не подключается** ни к одному правилу — только компилируется и типизируется
- [ ] прогнать `yarn typecheck` и `yarn lint` — новый файл проходит проверки при том, что не
