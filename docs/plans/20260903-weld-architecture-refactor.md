...
- [x] сверить раздел «Алгоритм» в `docs/rules/no-barrel-bypass.md` с новой раскладкой файлов —
      раздел описывает алгоритм только именами функций (`parseSpecifier`, `findBarrier`,
      `renderSpecifier`, `hasEntryPoint`); конкретные имена файлов/путей (`barrier.ts`,
      `specifier.ts`, `check.ts`) в документации не упоминаются вовсе — правок не потребовалось
- [x] проверить, что раздел `settings.weld` документации не противоречит новому `weld.ts`-фасаду —
      сверено с `src/settings/weld.ts` и `src/settings/aliases.ts`: валидация (`settings.weld`/
      `aliases` не объект → исключение), нормализация записей и кэш по `WeakMap` ведут себя как
      описано в документации; поведение чтения алиасов не изменилось, текст остался верным как есть,
      правок не потребовалось
- [x] проверить раздел CLAUDE.md «Добавление правила» — добавлено упоминание раскладки
      `src/rules/<имя>/core/` + `pipeline.ts` + `index.ts` как сложившегося (для `no-barrel-bypass`)
      варианта для правила с несколькими проверками, наравне с уже описанной раскладкой
      `src/settings/`/`src/env/`
- [x] пункты выше явно отмечены с указанием, где правки не потребовались и почему
