...
автоматически подхватываются из `compilerOptions.paths` ближайшего tsconfig с настроенным именем (по
умолчанию `tsconfig.json`, с резолвом `extends`); задать их явно можно через `settings.weld`:
